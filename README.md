...
Moneta Gateway PHP SDK is a small library of PHP code that you can use to quickly integrate with the Moneta Gateway system and submit transactions, check their status and more.
...
Before using the Moneta Gateway PHP SDK you should be familiar with the contents of the [API Specification for Merchants](docs/API-Specification.pdf) document as it describes all fields and their meaning within a given payment transaction.

## Setup your Project

Moneta Gateway PHP SDK is delivered as a composer(*) package, or dependency, that you should add to your PHP project in order to use it.
